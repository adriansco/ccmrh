...
@section('breadcrumbs')
    <div class="page-head">
        <h2 class="page-head-title">Modificar</h2>
        ...
        <div class="col-lg-12">
            <div class="card card-border-color card-border-color-primary">
                <div class="card-header card-header-divider">Datos
                    {{ $condition->name }}<span class="card-subtitle">Los campos
                        marcados con (*) son obligatorios.</span></div>
                <div class="card-body">
                    <form action="{{ route('conditions.update', $condition) }}" method="POST" class="row g-3">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-12 pt-3">
                            <p class="text-right">
                                ...
                                <a href="{{ route('conditions.index') }}" class="btn btn-space btn-danger">Cancel</a>
                            </p>
                        </div>
                    </form>
                </div>
