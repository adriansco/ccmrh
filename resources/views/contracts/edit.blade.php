...
@section('breadcrumbs')
    <div class="page-head">
        <h2 class="page-head-title">Modificar rol</h2>
        <nav aria-label="breadcrumb" role="navigation">
            <ol class="breadcrumb page-head-nav">
                <li class="breadcrumb-item"><a href="{{ route('roles.index') }}">Roles</a></li>
                {{-- <li class="breadcrumb-item"><a href="#">Tables</a></li> --}}
                <li class="breadcrumb-item active">Detalles rol</li>
            </ol>
        </nav>
    </div>
@endsection
        ...
        <div class="col-lg-12">
            <div class="card card-border-color card-border-color-primary">
                <div class="card-header card-header-divider">Datos
                    {{ $role->name }}<span class="card-subtitle">Los campos
                    ...
                    <form action="{{ route('roles.update', $role) }}" method="POST" class="row g-3">
                        ...
                        <div class="col-md-12 form-group">
                            ...
                            <div class="form-group">
                                <select name="team[]" id="team" class="tags selectpicker" multiple>
                                    @foreach ($permission as $tdrop => $id)
                                        @if (old('team'))
                                            <option value="{{ $id->id }}"
                                        ...
                                        @else
                                            <option value="{{ $id->id }}"
                                                {{ $role->permissions->contains($id->id) ? 'selected' : '' }}>
                                                {{ $id->name }}
                                            </option>
                                        @endif
                                    @endforeach
                        ...
                        <div class="col-12 pt-3">
                            <p class="text-right">
                                ...
                                <a href="{{ route('roles.index') }}" class="btn btn-space btn-danger">Cancel</a>
                            </p>
                        </div>
                    </form>
                </div>
